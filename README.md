...
Refer to the [examples](https://github.com/supabase/etl/tree/main/etl/examples) folder to run examples for destinations other than `stdout` (currently only `bigquery` and `duckdb` supported). A quick tip: to see all the command line options, run the example wihout any options specified, e.g. `cargo run --example bigquery` will print the detailed usage instructions for the `bigquery` destination.
...
Each destination is behind a feature of the same name, so remember to enable the right feature. The git dependency is needed for now because `etl` is not yet published on crates.io. You'd also need to add a dependency to tokio:
...
The `etl` crate has the following features:
...
To run the `etl` examples from the root of the repository, use the following command:

`cargo run -p etl --example <example_name> --features="<example_name>" ...`

In the above command we ask cargo to run the examples from the `etl` crate in the workspace. We also enable the right features needed for the example. Usually the feature is named the same as the example. E.g.:

`cargo run -p etl --example duckdb --features="duckdb"`

If you are inside the `etl` folder inside the root, then you can omit the `-p etl` part from the command.
...
- `api` - REST api used for hosting `etl` in a cloud environment.
- `etl` - The main library crate containing the core logic.
- `replicator` - A binary crate using `etl`. Packaged as a docker container for use in cloud hosting.

## Roadmap

`etl` is still under heavy development so expect bugs and papercuts but overtime we plan to add the following destinations.
