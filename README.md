...
```
cargo run -p supabase_etl --example stdout --features="stdout" -- --db-host localhost --db-port 5432 --db-name postgres --db-username postgres --db-password password cdc my_publication stdout_slot
```

In the above example, `supabase_etl` connects to a Postgres database named `postgres` running on `localhost:5432` with a username `postgres` and password `password`. The slot name `stdout_slot` will be created by `supabase_etl` automatically.

Refer to the [examples](https://github.com/supabase/supabase_etl/tree/main/supabase_etl/examples) folder to run examples for destinations other than `stdout` (currently only `bigquery` and `duckdb` supported). A quick tip: to see all the command line options, run the example wihout any options specified, e.g. `cargo run --example bigquery` will print the detailed usage instructions for the `bigquery` destination.

## Getting Started

To use `supabase_etl` in your Rust project, add it via a git dependency in `Cargo.toml`:

```toml
[dependencies]
supabase_etl = { git = "https://github.com/supabase/supabase_etl", features = ["stdout"] }
```

Each destination is behind a feature of the same name, so remember to enable the right feature. The git dependency is needed for now because `supabase_etl` is not yet published on crates.io. You'd also need to add a dependency to tokio:
...
For more examples, please refer to the [examples](https://github.com/imor/supabase_etl/tree/main/supabase_etl/examples) folder in the source.

## Features

The `supabase_etl` crate has the following features:
...
To run the `supabase_etl` examples from the root of the repository, use the following command:

`cargo run -p supabase_etl --example <example_name> --features="<example_name>" ...`

In the above command we ask cargo to run the examples from the `supabase_etl` crate in the workspace. We also enable the right features needed for the example. Usually the feature is named the same as the example. E.g.:

`cargo run -p supabase_etl --example duckdb --features="duckdb"`

If you are inside the `supabase_etl` folder inside the root, then you can omit the `-p supabase_etl` part from the command.
...
- `api` - REST api used for hosting `supabase_etl` in a cloud environment.
- `supabase_etl` - The main library crate containing the core logic.
- `replicator` - A binary crate using `supabase_etl`. Packaged as a docker container for use in cloud hosting.

## Roadmap

`supabase_etl` is still under heavy development so expect bugs and papercuts but overtime we plan to add the following destinations.
...
See the [open issues](https://github.com/imor/supabase_etl/issues) for a full list of proposed features (and known issues).
...
Applications can use data sources and destinations from `supabase_etl` to build a data pipeline to continually copy data from the source to the destination. For example, a data pipeline to copy data from Postgres to DuckDB takes about 100 lines of Rust.
...
Of course, the real code is more than these four lines, but this is the basic idea. For a complete example look at the [duckdb example](https://github.com/imor/supabase_etl/blob/main/supabase_etl/examples/duckdb.rs).

### Data Sources

A data source is the source for data which the pipeline will copy to the data destination. Currently, the repository has only one data source: [`PostgresSource`](https://github.com/imor/supabase_etl/blob/main/supabase_etl/src/pipeline/sources/postgres.rs). `PostgresSource` is the primary data source; data in any other source or destination would have originated from it.
